...
import { type InsertUser, type PublicUser, type SelectUser, user } from "~/server/db/schema";
...
async function doesEmailExists(userEmail: string): Promise<boolean> {
    const usr = await db.select({email: user.email}).from(user).where(eq(user.email, userEmail));
...
export async function getUserByEmail(userEmail: string): Promise<Result<PublicUserType>> {
    const usr = await db.select({
        ...
        sex: user.sex,
        dob: user.dob,
    }).from(user).where(eq(user.email, userEmail));
...
export async function getUserByUserId(userId: string): Promise<Result<PublicUserType>> {
    const usr = await db.select({
        id: user.id,
        firstName: user.firstName,
        middleName: user.middleName,
...
export async function validateUserByEmail(userEmail: string, password: string): Promise<Result<string>> {
    const usr = await db.select({
        id: user.id,
        email: user.email, 
